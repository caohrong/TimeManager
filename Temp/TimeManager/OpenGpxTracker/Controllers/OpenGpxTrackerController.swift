...
class OpenGpxTrackerController: UIViewController, UIGestureRecognizerDelegate {
    ...
    var gpxTrackingStatus: GpxTrackingStatus = .notStarted {
        didSet {
            ...
            case .paused:
                ...
                trackerButton.setTitle("Resume", for: UIControl.State())
                trackerButton.backgroundColor = UIColor.green
    ...
    init() {
        ...
        signalAccuracyLabel = UILabel()
        coordsLabel = UILabel()
        timeLabel = UILabel()
        speedLabel = UILabel()
        totalTrackedDistanceLabel = UIDistanceLabel()
        currentSegmentDistanceLabel = UIDistanceLabel()

        followUserButton = UIButton()
        newPinButton = UIButton()
        folderButton = UIButton()
        resetButton = UIButton()
        aboutButton = UIButton()
        preferencesButton = UIButton()
        shareButton = UIButton()
        trackerButton = UIButton()
        saveButton = UIButton()
        followUser = true
    ...
    override func viewDidLoad() {
        ...
        let mapH: CGFloat = view.bounds.size.height - (isIPhoneX ? 0.0 : 20.0)
        map.frame = CGRect(x: 0.0, y: isIPhoneX ? 0.0 : 20.0, width: view.bounds.size.width, height: mapH)
...
//         Set default zoom
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        guard let baseURL = URL(string: "file:///Users/huanrong/Library/Developer/CoreSimulator/Devices/18A990FD-F3A1-44CE-9B75-2A45990F3BE6/data/Containers/Data/Application/7090B6BB-DA9F-4643-BB9A-CDB64580C1A1/Documents") else {
            return
        }
...
//        self.didLoadGPXFileWithName(<#T##gpxFilename: String##String#>, gpxRoot: <#T##GPXRoot#>)
    ...
    @objc func openFolderViewController() {
        ...
        present(navController, animated: true) { () in }
    }
    ...
    @objc func openShare() {
        ...
        let gpxString: String = map.exportToGPXString()
    ...
    @objc func addPinAtTappedLocation(_ gesture: UILongPressGestureRecognizer) {
        if gesture.state == UIGestureRecognizer.State.began {
            ...
            let point: CGPoint = gesture.location(in: map)
    ...
    func pinchGesture(_: UIPinchGestureRecognizer) {
        print("pinchGesture")
    ...
    @objc func saveButtonTapped() {
        ...
        if gpxTrackingStatus == .notStarted, !hasWaypoints {
            return
        }
...
extension OpenGpxTrackerController: PreferencesTableViewControllerDelegate {
    ...
    func didUpdateTileServer(_ newGpxTileServer: Int) {
        ...
        map.tileServer = GPXTileServer(rawValue: newGpxTileServer)!
    }
...
extension OpenGpxTrackerController: CLLocationManagerDelegate {
    ...
    func locationManager(_: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        ...
        signalImageView.image = UIImage(named: "signal\(newLocation.singleAccuracyLevel)")
